#!/usr/bin/env bash
# This script installs Uppaal launcher shortcut for the Linux desktop environment.
set -e

HERE="$( cd "$( dirname "${BASH_SOURCE[0]}" )" && pwd )"
VERSION=$("${HERE}/bin/verifyta" -v | head -n 1 | awk '{ print $2 }')
NAME="Uppaal-${VERSION}"
SCRIPT="$0"
...
for c in xdg-mime xdg-icon-resource xdg-desktop-icon xdg-desktop-menu update-desktop-database; do
    testcommand "${c}"
done
echo "OK"
...
echo "OK"
echo "The launcher is installed in ${LAUNCHER}"
echo "In order to uninstall the launcher, simply delete ${LAUNCHER}"
